...
import { YupSchemaProvider, useYupFieldProps } from './index';

describe('useYupFieldProps', () => {
  const schema = object({
    name: string().required().min(3).max(50).matches(/^[a-zA-Z]+$/, { excludeEmptyString: true }).email().url().uuid().datetime({ allowOffset: true, precision: 2 }),
    age: number().required().min(18).max(65).integer().default(30),
    ageExclusive: number().nullable().lessThan(70).moreThan(10),
    birthdate: date().required().min(new Date('2000-01-01')).max(new Date('2020-12-31')),
    tags: array().of(string().required()).min(1).max(5),
  });

  const values = { name: 'John', age: 30, birthdate: new Date('2000-05-15'), tags: ['tag1', 'tag2'] };
  ...
  it('returns the correct field properties for a number field part 2', () => {
    const { result } = renderHook(() => useYupFieldProps('ageExclusive'), { wrapper });
  ...
  it('returns the correct field properties for a date field', () => {
    const { result } = renderHook(() => useYupFieldProps('birthdate'), { wrapper });
